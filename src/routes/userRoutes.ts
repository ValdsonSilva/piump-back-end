...
// import { requireAuth, requireUserAccountType, signToken } from '../middleware/auth.js';
...
// usersRouter.get('/route', requireAuth, requireUserAccountType('ADMIN'), async (req: Request, res: Response) => {
//   const date = String(req.query.date ?? '').trim();
//   if (!date) return res.status(400).json({ error: 'date=YYYY-MM-DD required' });
//   const groups = await listScheduledByZip(date);
